<div>
    <form wire:submit.prevent="new">
        <div class="grid sm:grid-cols-1 md:grid-cols-5 gap-4 m-2">
            <div class="mb-6">
            ...
            @if ($almacen_id>0)
                <div class="mb-6 col-span-2">
                    <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Seleccionar producto para el almacén: <strong class="uppercase">{{$almacen->name}}</strong> de la sede: <strong class="uppercase">{{$sede->name}}</strong> </label>
                    <div class="relative">
        ...
        @if ($tipo==="0" && $alumno_id===0 && $producto)
            <div class="mb-6 col-span-2">
                <div class="w-full">
        ...
        @endif
        @if ($tipo==="0" && $alumno_id>0 && $producto)
            <h4 class="font-semibold text-lg m-3 text-center bg-slate-200 p-3">Se generará sálida de productos para: <strong class="uppercase">{{$alumnoName}}</strong></h4>
        @endif
                    ...
                    <div class="ml-3 text-sm font-normal">
                        @if ($ultimoregistro)
                            <span class="mb-1 text-xl font-semibold text-gray-900 dark:text-white capitalize">
                                último movimiento para <strong class="uppercase">{{$producto->name}}</strong> en el almacén <strong class="uppercase">{{$almacen->name}}</strong> de la sede <strong class="uppercase">{{$sede->name}}</strong>
                            </span>
                                ...
                                <div>
                                    <ul role="list" class="space-y-5 my-7">
                                        <li class="flex space-x-3 items-center">
                                            ...
                                            <span class="bg-blue-200 text-black text-lg font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-green-900 dark:text-green-300">{{$ultimoregistro->cantidad}}</span>
                                        </li>
                                        <li class="flex space-x-3 items-center">
                                            <span class="text-base font-normal leading-tight text-gray-500 dark:text-gray-400">Saldo: </span>
                                            <span class="bg-blue-200 text-black text-lg font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-green-900 dark:text-green-600">{{$ultimoregistro->saldo }}</span>
                                        </li>
                                        <li class="flex space-x-3 items-center">
                                            <span class="text-base font-normal leading-tight text-gray-500 dark:text-gray-400">Precio: </span>
                                            <span class="bg-blue-200 text-black text-lg font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-green-900 dark:text-green-600">$ {{ number_format($ultimoregistro->precio, 0, '.', ' ')}}</span>
                                        </li>
                                    </ul>
                                </div>
                                <div>
                        ...
                        @else
                            <span class="mb-1 text-sm font-semibold text-gray-900 dark:text-white capitalize">
                                ¡NO TIENE MOVIMIENTOS! El producto: <strong class="uppercase">{{$producto->name}}</strong> en el almacén: <strong class="uppercase">{{$almacen->name}}</strong> de la sede: <strong class="uppercase">{{$sede->name}}</strong>
                            </span>
                        @endif
                    </div>
                </div>
        ...
        @if ($almacen_id>0)
            <div class="grid grid-cols-2 gap-3 bg-slate-300 m-3 p-3">
                <div class="grid grid-cols-3 gap-3 m-1 p-1">
